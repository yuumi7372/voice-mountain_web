  ...
  useEffect(() => {
    async function fetchMountains() {
      try {
        ...
        // 山データとタグを結びつける
        const posts: MountainPost[] = (mountainData ?? []).map((mountain) => {
            ...
            return {
              ...
              //aiComment: mountain.analysis_data?.aiReview?.comment ?? "",
      ...
      <div className={styles.wrapper}>
        <header className={styles.header}>
          <div>
            ...
            <p className={styles.subtitle}>たった5秒で山ができる</p>
          </div>
        ...
        {isLoading ? (
          <section className={styles.emptyArea}>
            ...
            <h2 className={styles.emptyTitle}>山を読み込んでいます</h2>
          </section>
        ) : mountains.length === 0 ? (
          <section className={styles.emptyArea}>
            <div className={styles.emptyIcon}>⛰️</div>
            <h2 className={styles.emptyTitle}>まだ山がありません</h2>
            <p className={styles.emptyText}>最初の声の山を作ってみよう</p>
          </section>
        ) : (
          <section className={styles.grid}>
            {mountains.map((mountain) => (
              <article key={mountain.id} className={styles.card} onClick={() => router.push(`/mountain/${mountain.id}`)}>
                <div className={styles.imageArea}>
                  {mountain.image ? (
                    <img src={mountain.image} alt={mountain.title} className={styles.thumbnail}/>
                  ) : (
                ...
                <div className={styles.cardBody}>
                  {/* タグ */}
                  {mountain.tagName && (<p className={styles.tag}># {mountain.tagName}</p>)}
